...
function getEnv(envvar) {
	...
	debug$7("%s=%s", envvar, val);
	const result = typeof val === "string" && val.length > 0 ? val : void 0;
...
async function scad2stl(launchPath, files) {
	...
	let scadProcess;
	try {
		const dirResult = await mkdirForFileAsync(files.out);
		console.error(dirResult);
		if (!dirResult) throw new Error(`There was an error creating the directory for ${files.out}`);
		...
		scadProcess.on("spawn", () => {
			debug$3.extend("spawn")(launchPath);
			timer.start();
		});
...
function spawnOpenSCAD(files, launchPath) {
	...
	const args = makeScadArgs(inFile, outFile);
	debug$3.extend("spawn")("openscad args: %O", args);
	return spawn(launchPath, args, { cwd: files.cwd });
}
/**
...
function spawnDockerOpenSCAD(files, tag) {
	...
	debug$3.extend("spawn")(spawnArgs);
...
const createStringBoolSchema = (opts) => {
	return z.preprocess((val) => val ?? env[opts.envvar], StringBoolSchema).default(opts.default);
};
const PluginOptionsSchema = z.object({
	launchPath: z.preprocess((val) => {
		if (val === null || val === "auto" || val === "nightly") return autoBinPath(platform, val ?? "auto");
		return val;
	}, z.string().optional()),
	theme: z.optional(z.enum(THEMES)).superRefine((val, ctx) => {
